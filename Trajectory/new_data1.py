...
# -*- coding: utf-8 -*-
"""
Created on Fri May 21 02:36:14 2021
...
save_path = '/home/ogk/Documents/OGK/new_data/'
...
for session in range(1, 38):
    ...
    nonlost_ch = np.zeros([len(Unsort_data), len(ok_ch)])
    for i in range(len(ok_ch)):
        nonlost_ch[:, i] = Unsort_data[:, ok_ch[i]]
    avg_fr = np.mean(nonlost_ch, axis=1)
    for i in lost_ch:
        Unsort_data[:, i] = avg_fr
    
    np.savez(save_path+'B'+str(session), fr=Unsort_data, t=time_bin, vel=velocity)
